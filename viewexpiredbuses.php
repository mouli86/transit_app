...
<body>
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
    ...
    <?php
     ...
     $driverQuery = "SELECT * FROM BUS WHERE LicenseExpiryDate < '2038-10-10'";
$driverResult = $conn->query($driverQuery);

// Display results
echo "<h2>Expired Bus's Licenses</h2>
<table class='table table-hover table-bordered table-striped' align='center'>
                ...
                $query = mysqli_query($conn, "SELECT * FROM BUS WHERE LicenseExpiryDate < CURDATE()") or die(mysqli_error($conn));
    ...
    <div style="margin:30px;">
        <?php
        $driverQuery = "SELECT *, TIMESTAMPDIFF(YEAR, CURDATE(), LicenseExpiryDate) AS ExpireIn FROM BUS WHERE TIMESTAMPDIFF(YEAR, CURDATE(), LicenseExpiryDate) >= 0 ORDER BY ExpireIn ASC ;";
$driverResult = $conn->query($driverQuery);
echo "<h2>Bus's Licenses Expiring Soon</h2>
<table class='table table-hover table-bordered table-striped' align='center'>
        <tr>
        <th>Bus Number</th>
        <th>License Number</th>
